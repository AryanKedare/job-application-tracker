...
const STATUS_STYLE: Record<JobApplication['status'], { pill: string; dot: string; label: string }> = {
  Bookmarked:   { pill: 'bg-slate-700/60 text-slate-300 border border-slate-600/50',            dot: 'bg-slate-400',    label: 'Bookmarked' },
  Applied:      { pill: 'bg-blue-500/15 text-blue-300 border border-blue-500/30',               dot: 'bg-blue-400',     label: 'Applied' },
  Interviewing: { pill: 'bg-yellow-500/15 text-yellow-300 border border-yellow-500/30',         dot: 'bg-yellow-400',   label: 'Interviewing' },
  Offer:        { pill: 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/30',      dot: 'bg-emerald-400',  label: 'Offer' },
  Rejected:     { pill: 'bg-red-500/15 text-red-300 border border-red-500/30',                  dot: 'bg-red-400',      label: 'Rejected' },
  Ghosted:      { pill: 'bg-slate-600/30 text-slate-400 border border-slate-600/30',            dot: 'bg-slate-500',    label: 'Ghosted' },
}
...
export default function JobsTablePage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      else { setUserId(user.id); setAuthChecked(true) }
    }
    checkAuth()
  }, [router])
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (error) { showToast('Failed to delete application.', 'error'); fetchJobs() }
    else showToast(`Deleted "${job.job_title}".`, 'success')
  }

  const handleAddSuccess = (newJob: JobApplication) => { setJobs((prev) => [newJob, ...prev]); showToast('Application added!', 'success') }
                    ...
                    {filteredJobs.map((job) => (
                      <tr key={job.id} className="group hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="px-5 py-4 text-slate-500 whitespace-nowrap text-xs">
                          {job.date_applied
                            ? new Date(job.date_applied).toLocaleDateString('en-IE', { day: '2-digit', month: 'short' })
                            : '—'}
                        </td>
